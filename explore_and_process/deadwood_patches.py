"""Whole-scene mask construction, grid tiling and splitting for deadwood fine-tuning.

Separate from the crown pipeline's tile_patches because the label mask carries
two classes built from two vector layers, and because the split is assigned per
tile rather than inherited from a scene-wide mask.

Tiles are disjoint on the ground. That is what makes a random split honest here
and what removed the per-split masks the crown-centred crops needed: crops
centred on crowns 7-12 m apart overlapped heavily, so a held-out crown fell
inside a training crop and had to be blanked out of it.
"""
